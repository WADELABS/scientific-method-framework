...
class DeepScientificAgent(ScientificAgent):
    ...
    def _is_hypothesis_compatible_with_paradigm(self, hypothesis: Hypothesis,
        ...
        if paradigm == ScientificParadigm.FALSIFICATIONISM:
            ...
            # any(word in hypothesis.statement.lower() for word in ["understand", "interpret", "meaning", "context"])
            return True
    ...
    async def _integrate_paradigm_experiments(self,
                                              paradigm_experiments: Dict[ScientificParadigm,
    ...
    async def _create_integrated_experiment(
            self, hypothesis_id: str, paradigm_experiments: List[Tuple[ScientificParadigm, Experiment]]) -> Experiment:
        ...
        if experiment.analysis:
            for test, results in experiment.analysis.items():
                if not isinstance(results, dict):
                    continue  # skip non-dict results for this check
